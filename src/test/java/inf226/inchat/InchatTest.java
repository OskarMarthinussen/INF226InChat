...
public class InchatTest{
    ...
    void chatSetup() throws Maybe.NothingException,SQLException {
        ...
        Stored<Session> aliceSession = inchat.register(new UserName("Alice"),new Password("badpassword", false)).get();
        inchat.register(new UserName("Bob"),new Password("worse", false)).get();
        Stored<Session> bobSession = inchat.login(new UserName("Bob"),new Password("worse", false)).get();
