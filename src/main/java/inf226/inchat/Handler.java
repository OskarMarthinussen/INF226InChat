...
public class Handler extends AbstractHandler
{
  ...
  private String sanitizeHTML(String unsafeHTML) {
    ...
    throws IOException, ServletException
  {
    ...
    if(request.getParameter("register") != null) {
        ...
        try {
            UserName username = new UserName((new Maybe<String>
                (request.getParameter("username"))).get());
            Password password = new Password((new Maybe<String>
                (request.getParameter("password"))).get(), false); //Password is not hashed
            System.err.println("Registering user: \"" + username.get()
        ...
        try {
            final UserName username = new UserName((new Maybe<String>
                (request.getParameter("username"))).get());
            System.err.println("Username: " + username.get());
            final Password password = new Password((new Maybe<String>
                (request.getParameter("password"))).get(), false); //Password is not hashed
    ...
    try {
        ...
        Cookie sessionCookie = new Cookie("session",session.identity.toString());
        ...
        response.setHeader("Set-Cookie", "session=" + sessionCookie.getValue() + "; HttpOnly; SameSite=strict");
        //response.addHeader("Content-Security-Policy", "script-src unsafe-inline");
        //response.addHeader("Content-Security-Policy", "script-src http://localhost:8080/script.js");
        ...
        try {
            if(target.startsWith("/channel/")) {
                ...
                String safeAlias = sanitizeHTML(alias);

                out.println("<!DOCTYPE html>");
                out.println("<html lang=\"en-GB\">");
                printStandardHead(out, "inChat: " + safeAlias);
                out.println("<body>");
                printStandardTop(out,  "inChat: " + safeAlias);
                out.println("<div class=\"main\">");
                printChannelList(out, account.value, safeAlias);
                printChannel(out, channel, safeAlias);
                out.println("</div>");
                out.println("</body>");
                out.println("</html>");
            ...
            if(target.equals("/joinChannel")) {
                out.println("<!DOCTYPE html>");
                out.println("<html lang=\"en-GB\">");
                printStandardHead(out, "inChat: " + account.value.user.value.username.get());
                out.println("<body>");
            ...
            if(target.startsWith("/editMessage")) {
                String alias = sanitizeHTML((new Maybe<String>
                        (request.getParameter("channelname"))).get());
                String messageid = sanitizeHTML((new Maybe<String>
                        (request.getParameter("message"))).get());
                String originalContent = sanitizeHTML((new Maybe<String>
            ...
            if(target.equals("/")) {
                ...
                printStandardHead(out, "inChat: " + account.value.user.value.username.get());
                out.println("<body>");
                printStandardTop(out, "inChat: " + account.value.user.value.username.get());
                out.println("<div class=\"main\">");
    ...
    private void printStandardHead(PrintWriter out, String title) {
        ...
        out.println("<title>" + sanitizeHTML(title) + "</title>");
        out.println("</head>");
    }
    ...
    private void printStandardTop(PrintWriter out, String topic) {
        out.println("<h1 class=\"topic\"><a style=\"color: black;\" href=\"/\">"+ sanitizeHTML(topic) + "</a></h1>");
    ...
    private void printChannelList(PrintWriter out, Account account, String current) {
        ...
        account.channels.forEach( entry -> {
            out.println("<li> <a href=\"/channel/" + sanitizeHTML(entry.first) + "\">" + sanitizeHTML(entry.first) + "</a></li>");
        });
        out.println("</ul>");
        out.println("</aside>");
    }
    ...
    **/
    private void printChannel(PrintWriter out,
        ...
        out.println("<form class=\"entry\" action=\"/channel/" + safeAlias + "\" method=\"post\">");
        out.println("  <div class=\"user\">You</div>");
        ...
        out.println("<h4>Set permissions</h4><form action=\"/channel/" + safeAlias + "\" method=\"post\">");
        ...
        return (e -> {
            switch(e.value.type) {
                case message:
                    ...
                    out.println("    <div class=\"user\">" + sanitizeHTML(e.value.sender) + "</div>");
                    out.println("    <div class=\"text\">" + sanitizeHTML(e.value.message));
                    out.println("    </div>");
                    out.println("    <div class=\"messagecontrols\">");
                    out.println("        <form style=\"grid-area: delete;\" action=\"/channel/" + sanitizeHTML(channel.value.name) + "\" method=\"POST\">");
                    ...
                    out.println("        ");
                    out.println("        <input type=\"hidden\" name=\"message\" value=\""+ e.identity + "\">");
                    out.println("        <input type=\"hidden\" name=\"channelname\" value=\""+ sanitizeHTML(channel.value.name) + "\">");
                    ...
                    return;
                case join:
                    out.println("<p>" + formatter.format(e.value.time) + " " + sanitizeHTML(e.value.sender) + " has joined!</p>");
                    return;
            }
        });
    }
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    try{
        ...
        try {
            final Stored<Session> admin = inchat.register(new UserName("admin"),new Password("pa$$w0rd", true)).get();
