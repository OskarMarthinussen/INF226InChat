...
public class InChat {
     ...
     * @param password This password is not hashed.
     */
    public Maybe<Stored<Session>> login(final UserName username,
        ...
        return atomic(result -> {
            ...
            if (!(!account.value.password.checkPassword(password.get()) && !(password.get().length() > 1000))) { 
    ...
    public Maybe<Stored<Session>> register(final UserName username,
                                           final Password password) {
        return atomic(result -> {
            if (password.checkNIST()){    
                final Stored<User> user =
                    userStore.save(User.create(username));
                final Stored<Account> account =
                accountStore.save(Account.create(user, new Password(password.get(), true))); // Password is hashed
                final Stored<Session> session =
                    sessionStore.save(new Session(account, Instant.now().plusSeconds(60*60*24)));
                result.accept(session); 
            } else System.err.println("Password was not NIST approved");
        });
    }
    ...
    public Maybe<Stored<Channel>> createChannel(Stored<Account> account,
        ...
        return atomic(result -> {
            ...
            //channelStore.setPermission(channel, account, "Owner");
            ...
            Stored<Channel.Event> joinEvent
                = channelStore.eventStore.save(
                    Channel.Event.createJoinEvent(channelID,
                        Instant.now(),
                        account.value.user.value.username.get()));
            //channelStore.setPermission(channel, account, "Participant");
            result.accept(
    ...
    public Integer permissionRank(String permission){
        switch (permission) {
            case "Owner":
                return 1;
            case "Moderator":
        ...
        return atomic(result -> {
                ...
                = channelStore.eventStore.save(
                    Channel.Event.createMessageEvent(channel.identity,Instant.now(),
                        account.value.user.value.username.get(), message));
            result.accept (
                Util.updateSingle(channel,
                                    channelStore,
...
/* 
    public Maybe<Stored<Channel>> postMessage(Stored<Account> account,
        ...
        return atomic(result -> {
            try {
                final String permission = channelStore.getPermission(channel, account);
                if (permissionRank(permission) <= 2 ){
                    Stored<Channel.Event> event
                = channelStore.eventStore.save(
                    Channel.Event.createMessageEvent(channel.identity,Instant.now(),
                        account.value.user.value.username.get(), message));
